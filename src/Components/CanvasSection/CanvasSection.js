...
export default class CanvasSection extends Component {
    ...
    componentDidMount() {
        ...
        if (queryParams.has("test") === true && this.props.test_load) {
            console.log("%c Automatic Test Mode ", "background-color: black; color: white");
    ...
    downloadCanvas() {
        ...
        fileName = "stegojs_" + (Math.random() * 1000000).toFixed() + "_" + fileName + ".bmp";
        let canvas = this._canvas;
    ...
    render() {
        const fileInput = (this.props.disableInput) ? ("") :
            (<div>
        ...
        const clearButton = this.props.clear &&
            <button className="waves-effect waves-light btn yellow darken-1 black-text"
        ...
        return (
            <div>
                ...
                <div hidden={this.props.hideCanvas}
                    className={"section-content scale-transition scale-" + this.state.scale} >
                    <label className="label"
                        htmlFor={this.props.id + "-canvas"}>
