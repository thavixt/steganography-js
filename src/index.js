...
const basePath = "/projects/steganography-js/";
...
const check = () => {
    ...
    //console.time("Running checks");

    // Check for API support
    if (!(window.Worker && window.Promise && window.fetch &&
        window.File && window.FileReader && window.FileList &&
        window.Blob && window.TextDecoder)) {
        alert("The required APIs are not fully supported in this browser. \rTry using the latest version of Chrome, Edge or Firefox.");
        // Log required APIs
        console.error("The required APIs are not fully supported in this browser.\nTry using the latest version of Chrome, Edge or Firefox.");
        console.error("The required APIs are:\nBlob, fetch, File, FileReader, FileList, Promise, TextDecoder, Worker");
    }
    ...
    /* if (navigator.deviceMemory < 1) {
        ...
        console.error("Low device memory detected:", navigator.deviceMemory);
    } */
    ...
    if (navigator.hardwareConcurrency < 2) {
        ...
        console.error("Low number of CPU cores/threads detected:", navigator.hardwareConcurrency);
    }
    ...
    //console.timeEnd("Running checks");
